...
class Game:
    ...
    def draw(self, screen):
        ...
        # --- Helwan Linux Customization: Draw Held Block ---
        if self.held_block:
            # You might need to adjust these coordinates for optimal placement
            # This is just a placeholder example, you will likely need a dedicated Rect for it
            # For simplicity, drawing it near the score/next area, maybe to the left or top.
            # Let's assume a dedicated area similar to 'Next Block' area
            if self.held_block.id == 3: # IBlock is horizontal, needs different offset
                self.held_block.draw(screen, 320 + 50, 440) # مثال: Adjust these values
            elif self.held_block.id == 4: # OBlock is 2x2, sometimes needs adjustment
                 self.held_block.draw(screen, 320 + 50, 430) # مثال: Adjust these values
            else:
                self.held_block.draw(screen, 320 + 50, 420) # مثال: Adjust these values
