...
class Game:
    ...
    def draw(self, screen):
        ...
        block_display_x = 360 # الإحداثي X لمنطقة عرض Next Block
        block_display_y = 290 # الإحداثي Y لمنطقة عرض Next Block

        if self.next_block.id == 3: # IBlock (خط مستقيم)
            self.next_block.draw(screen, block_display_x - 35, block_display_y + 10) # إزاحة خاصة لـ IBlock
        elif self.next_block.id == 4: # OBlock (مربع)
            self.next_block.draw(screen, block_display_x - 10, block_display_y + 10) # إزاحة خاصة لـ OBlock
        else: # باقي الكتل
            self.next_block.draw(screen, block_display_x, block_display_y) # الإزاحة الافتراضية
        
        # --- تعديل: إحداثيات رسم "Held Block" (القطعة المحفوظة) ---
        held_block_display_x = 360 # الإحداثي X لمنطقة عرض Held Block
        held_block_display_y = 480 # الإحداثي Y لمنطقة عرض Held Block

        if self.held_block:
            if self.held_block.id == 3: # IBlock
                self.held_block.draw(screen, held_block_display_x - 35, held_block_display_y + 10) # إزاحة خاصة
            elif self.held_block.id == 4: # OBlock
                 self.held_block.draw(screen, held_block_display_x - 10, held_block_display_y + 10) # إزاحة خاصة
            else: # باقي الكتل
                self.held_block.draw(screen, held_block_display_x, held_block_display_y) # الإزاحة الافتراضية
